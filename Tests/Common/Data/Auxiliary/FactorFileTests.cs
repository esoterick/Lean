...
namespace QuantConnect.Tests.Common.Data.Auxiliary
{
    ...
    public class FactorFileTests
    {
        ...
        public void GetsSplitsAndDividends()
        {
            ...
            Assert.AreEqual(0.73m, dividend.Distribution);
        ...
        public void AppliesDividend()
        {
            ...
            {
                Console.WriteLine($"expected: {item.expected} actual: {item.actual}  diff: {100* (1 - item.actual.PriceFactor/item.expected.PriceFactor):0.00}%");
                ...
                var delta = item.expected.PriceFactor * 0.02m;
                Assert.AreEqual((double)item.expected.PriceFactor, (double)item.actual.PriceFactor, (double)delta);  // allow up to 2% deviation here
                Assert.AreEqual((double)item.expected.SplitFactor, (double)item.actual.SplitFactor);
                delta = item.expected.PriceScaleFactor * 0.02m;
                Assert.AreEqual((double)item.expected.PriceScaleFactor, (double)item.actual.PriceScaleFactor, (double)delta);
            }
        }
